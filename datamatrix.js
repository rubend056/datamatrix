...
export default function DATAMatrix(Q) {
	...
	return (function () {
		...
		if (!hasDOM) {
			// No DOM (e.g., Node.js) — return an SVG string
			const viewBox = `0 0 ${sx} ${sy}`;
			const width = (dm / sy * sx) | 0;
			const height = dm;

			const background = `<path fill="${bg}" d="M0,0v${sy}h${sx}V0H0Z"/>`;
			const foreground = `<path transform="matrix(${mx.join(',')})" d="${path}"/>`;

			return (
				`<svg viewBox="${viewBox}" width="${width}" height="${height}" ` +
				`fill="${fg}" shape-rendering="crispEdges" xmlns="http://www.w3.org/2000/svg">` +
				(bg ? background : "") + foreground +
				`</svg>`
			);
		}

		// DOM is available — build and return an SVG element (existing behavior)
		r = svg('svg', {
